...
class plgVmPaymentBancodobrasil extends vmPSPlugin {
    ...
    function __construct(& $subject, $config) {
        ...
        $varsToPush = array('payment_logos' => array('', 'char'),	
            ...
            'convenio' => array('', 'string'),
            'cobranca' => array('', 'string'),
			'dias_vencimento' => array('', 'int'),
            'convenio_teste' => array('', 'string'),
            'cobranca_teste' => array('', 'string'),
			'dias_vencimento_teste' => array('', 'int'),
			'mensagem_boleto' => array('', 'string'),
			'tipo_integracao' => array('', 'string'),
    ...
    function plgVmConfirmedOrder($cart, $order) {
        ...
        //$html .= $this->getHtmlRow('bancodobrasil_AMOUNT', $totalInPaymentCurrency.' '.$currency_code_3);
        $html .= '</table>' . "\n";
        ...
        $html .= '<form name="bancodobrasil" id="bancodobrasil" action="https://www16.bancodobrasil.com.br/site/mpag/" method="poBT">  ';
		$html .= "<input type='hidden' name='nome' value='". $order["details"]["BT"]->first_name . ' ' . $order["details"]["BT"]->last_name."' />";
		$html .= "<input type='hidden' name='endereco' value='". $order["details"]["BT"]->address_1 . ',' . $order["details"]["BT"]->address_2 ."' />";
		...
		/*
			...
			5 - BB Crediário Internet
		*/
		$tp_pagamento 	= $method->tipo_integracao; // alterar pra emitir a 2a via
		...
		$html .= "<input type='hidden' name='refTran' value='".$cobranca.str_pad($virtuemart_order_id, 9, "0", STR_PAD_LEFT)."' />";
		...
		$url_informa = JROUTE::_(JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginnotification') ;
		$html .= "<input type='hidden' name='urlInforma' value='".$url_informa."?order_id=".$order_id."' />";
		...
		$html .= "<input type='hidden' name='urlRetorno' value='".$url_retorno."?order_id=".$order_id."' />";
		...
		**/
		...
		$html .= '<input type="image" src="'.$url_imagem_pagamento.'" class="button" />';
...
// 		$params = new JParameter($payment->payment_params);
	...
	*/
	function plgVmOnPaymentNotification() {
		...
		if (!isset($bancodobrasil_data['id_transacao'])) {
			return;
		}		
	...
	//$preload=true   preload the data here too preserve not updated data
		...
		if (!(empty($error_msg) )) {
			 ...
			 *
			 */
			if (empty($bancodobrasil_data['cod_status']) || ($bancodobrasil_data['cod_status'] != '0' && $bancodobrasil_data['cod_status'] != '1' && $bancodobrasil_data['cod_status'] != '2')) {
			//return false;
			}
			$bancodobrasil_status = $bancodobrasil_data['cod_status'];
			if ($bancodobrasil_status == 1) {
				$new_status = $method->status_aprovado;
			} elseif ($bancodobrasil_status == 2) {
		...
		if ($virtuemart_order_id) {
			...
			$order['comments'] = 'O status do seu pedido '.$order_number.' no Pagamento Digital foi atualizado: '.utf8_encode($bancodobrasil_data['status']);
