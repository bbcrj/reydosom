...
<div class="ask-a-question-view">
	...
	<div class="form-field">
		...
		<form method="post" class="form-validate" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$this->product->virtuemart_product_id.'&virtuemart_category_id='.$this->product->virtuemart_category_id.'&tmpl=component') ; ?>" name="askform" id="askform">
			...
			<label><span><?php echo JText::_('COM_VIRTUEMART_USER_FORM_NAME')  ?> :</span> <input type="text" class="inputbox validate[required,minSize[4],maxSize[64]]" value="<?php echo $this->user->name ?>" name="name" id="name" size="30"  validation="required name"/></label>
			<label><span><?php echo JText::_('COM_VIRTUEMART_USER_FORM_EMAIL')  ?> : </span><input type="text" class="inputbox validate[required,custom[email]]" value="<?php echo $this->user->email ?>" name="email" id="email" size="30"  validation="required email"/></label>
			<div class="clear"></div>
			<label>
				<?php
