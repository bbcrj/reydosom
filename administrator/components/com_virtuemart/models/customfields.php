...
//// plugin
					...
					foreach ($group->options as $productCustom) {
						...
						$group->display .= '<div class="group"><input id="'.$productCustom->value.'" '.$checked.' type="radio" value="'.$productCustom->value.'" name="customPrice['.$row.']['.$group->virtuemart_custom_id.']" /><label for="'.$productCustom->value.'">'.$this->displayType($productCustom->custom_value,$group->field_type,0,'',$row,1).' '.$price.'</label></div>' ;
						$checked ='';
					}
				}
				$row++ ;
			...
			if ($selected) {
				...
				$productCustom = self::getProductCustomFieldCart ($product_id,$selected );
 				if ($productCustom->field_type == "E") {
					...
					$html .= '<div> <span>'.$productCustom->custom_title.' : </span>'.self::displayCustomMedia($productCustom->custom_value).'</div>';
				}else {

					$html .= '<div> <span>'.$productCustom->custom_title.' : </span>'.$productCustom->custom_value.'</div>';
				}
			}
			$row++;
		}
