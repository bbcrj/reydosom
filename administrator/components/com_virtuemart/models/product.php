		...
		if($group){
			...
			switch ($group) {
				case 'featured':
					...
					break;
				case 'latest':
					$date = JFactory::getDate( time()-(60*60*24*5000) ); //Set on a week, maybe make that configurable
...
function getOrderByList($virtuemart_category_id=false) {
	...
	unset ($getArray['globalCurrencyConverter'],$getArray['search'],$getArray['keyword'],$getArray['virtuemart_manufacturer_id'],$getArray['order'],$getArray['orderby']);
	...
	$orderByList ='<div class="title">'.JText::_('COM_VIRTUEMART_ORDERBY').':</div><div class="orderlistcontainer"><div class="activeOrder"><a title="'.$orderTxt.'" href="'.$link.'">'.JText::_('COM_VIRTUEMART_SEARCH_ORDER_'.$orderby).' '.$orderTxt.'</a></div>';
	$orderByList .= $orderByLink.'</div>';

	if (count($manufacturers)>= 1) {
	$manuList ='';
	if(VmConfig::get('show_manufacturers')){
		if (empty ($currentManufacturerLink) ) $currentManufacturerLink = '<div class="title Manufactur">'.JText::_('COM_VIRTUEMART_PRODUCT_DETAILS_MANUFACTURER_LBL').'</div><div class="activeOrder">'.JText::_('COM_VIRTUEMART_SEARCH_SELECT_MANUFACTURER').'</div>';
