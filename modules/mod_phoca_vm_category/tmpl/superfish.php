...
// All done in recursive function - don't foreach data again and save the server performance.
	...
	?>
<?php
function SFVmCategoryTree ($category_id, $active_category_id, $vendorId, $cache, $categoryModel, $p) {
	...
	if (!empty($categories)) {
		...
		foreach ($categories as $c) {
			...
			if ($level == 0) {
				$parentmenu++;
				echo  '<li class="level'.$level.''.$parent.''.$active.'"><a '.$drop.' href="'.$url.'" >'.$c->category_name.'</a>' . "\n";
			} else {
				echo  '<li class="level'.$level.''.$parent.''.$active.'" style="'.strip_tags($columnstyle).'"><a href="'.$url.'" >'.$c->category_name.'</a>' . "\n";
			}
			...
			if (isset($childCats) && !empty($childCats)) {
				...
				if ((int)$p['countlevels'] == (int)$level) {
					...
					SFVmCategoryTree($c->virtuemart_category_id, $active_category_id, $vendorId, $cache, $categoryModel, $p);
					$level--;
				}
			}
			echo  '</li>'."\n";
...
SFVmCategoryTree ($category_id, $active_category_id, $vendorId, $cache, $categoryModel, $p);
